...
public class PanelRegister extends JPanelTemplate {
	...
	private static final int insetExtremes = 20;
	...
	public PanelRegister(User user, PanelToggle panelToggle, SubPanelToggle subPanelToggle) { 
		...
		final JLabel labelPasswordConfirm = new BuilderLabel.LabelBuilder()
		...
		final JTextField textFieldPasswordConfirm = new BuilderTextField.TextFieldBuilder()
				.columns(20)
				.build();
		
		
		final JButton buttonConfirmRegistration = new BuilderButton.ButtonBuilder()
				.text("Register")
				
				.font(montserrat)
				.actioncommand("register")
				.actionListener(e -> {
//					PanelToggle.card.show(subPanelToggle, "panel_register");
				}).build();
		
		final MouseListener mouseAdapterLabelAlreadyRegister = new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {			
				...
				if (label.getText().equals("Already register? Log in!")) {
					PanelToggle.card.show(panelToggle, "panellogin");
				} 
				
			}
		};
		
		final JLabel labelAlreadyRegister = new BuilderLabel.LabelBuilder()
				.text("Already register? Log in!")
				.foreground(WHITE)
				.font(montserrat)
				.opaque(true)
				.mouseListener(mouseAdapterLabelAlreadyRegister) 
				.build();
		...
		super.add(labelPasswordConfirm);
		super.add(textFieldPasswordConfirm);
		
		super.add(buttonConfirmRegistration);
		super.add(labelAlreadyRegister);
